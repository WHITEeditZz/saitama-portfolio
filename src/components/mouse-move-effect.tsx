 ...
 * Layer 1: Neon Red (Primary) - Follows closely
 * Layer 2: Neon Dark Red (Secondary) - Follows with lag/parallax
 */
export default function MouseMoveEffect() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      layer1Ref.current.style.background = `radial-gradient(800px circle at ${p1.x}px ${p1.y}px, rgba(220, 38, 38, 0.12), transparent 60%)`;
      layer2Ref.current.style.background = `radial-gradient(600px circle at ${p2.x}px ${p2.y}px, rgba(153, 27, 27, 0.08), transparent 60%)`;
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-20%] left-[-20%] w-[80%] h-[80%] rounded-full bg-red-955/10 blur-[80px] animate-pulse-slow" />
        <div className="absolute bottom-[-20%] right-[-20%] w-[80%] h-[80%] rounded-full bg-rose-955/10 blur-[80px] animate-pulse-slow delay-1000" />
      </div>
    );
  }
